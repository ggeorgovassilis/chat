...
public interface IMessageDAO extends CrudRepository<Message,String>{
	...
	@Query("select m from Message m where m.recipient = ?1 and m.id > ?2 order by m.sent")
	List<Message> findUnreadMessagesForRecipient(String recipient, int minId);
	...
	List<Message> findUnreadMessagesBetweenUsers(String recipient, String sender, int minId);
}
